...
def train(cfg, line, agent, tain_model):
    print('开始训练！')
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while not done:
            ...
            # Memory_Buffer存储
            agent.memory.push(state_node.state.copy(), state_node.action.copy(), state_node.current_reward.copy(), state_node.next_state.copy(), done)
            if done:
        ...
        print('回合：{}/{}，奖励：{}, 能耗  {}, 牵引能耗  {}, 最终时间  {}, 最终速度  {}, 不安全次数  {}, 最终位置 {}'.format(i_ep + 1, cfg.train_eps, np.around(ep_reward[0], 2), np.around(total_power[0], 4), np.around(t_power[0], 4),
                                                                                                np.around(state_node.next_state[0], 2), np.around(state_node.next_state[1], 2), np.round(ep_unsafe_counts, 0),
                                                                                                state_node.step))
...
if __name__ == '__main__':
    cfg = PPOConfig()
    # train
    line, agent, train_model = env_agent_config(cfg, seed=4)  # 1和4都可以
    ...
    evalplot_speed(v_list, t_list, a_list, acc_list, limit_list, A_limit_list, tag="op_eval", env=cfg.env,
                   algo=cfg.algo,
